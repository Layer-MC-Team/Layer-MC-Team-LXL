...
void buffer_copier_init(context_t* context) {
    ...
    if(error != 0) {
        printf("LXL: error while initializing buffer-copier: %x\n", error);
        return;
    }
    copier->ready = true;
}
...
void glGetTexImage( 	GLenum target,
    ...
    return;
    unsupported_esver:
    printf("LXL: glGetTexImage only supported on OpenGL ES 3.1");
    return;
    unsupported:
    printf("LXL: unsupported parameters for glGetTexImage");
}
